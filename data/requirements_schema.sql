-- ============================================================================
-- SmartPR Requirement Monitoring & Fillable Documents Schema (Supabase / Postgres)
--
-- This schema powers the "Requirement Intelligence Layer":
--
--   1. Requirement Knowledge Base  — requirement_rules, requirement_documents
--   2. Form Template Engine        — form_templates, user_form_instances
--   3. Source Monitoring Agent     — requirement_monitoring_runs,
--                                     requirement_change_events
--
-- Design notes:
--   * All tables use `CREATE TABLE IF NOT EXISTS` and `ALTER TABLE ... ADD
--     COLUMN IF NOT EXISTS` so the schema can be applied idempotently per
--     process (mirrors data/graph_schema.sql + src/app/graph/store.ts).
--   * Requirements and templates are VERSIONED, never overwritten. A source
--     change creates a new draft row and flips the prior row's status; admin
--     approval publishes the new active version.
--   * Tables are intentionally generic (country/state/county/municipality) so
--     the knowledge base can expand from Puerto Rico to all US jurisdictions.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1. requirement_rules
--    One row per known requirement, keyed by jurisdiction + business/activity.
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS requirement_rules (
  id                    UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id             UUID,                         -- NULL = global/shared rule
...
-- ----------------------------------------------------------------------------
-- 2. requirement_documents
--    Source documents/forms attached to a rule (the raw artifacts).
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS requirement_documents (
  ...
  storage_path              TEXT,                       -- original file path inside Supabase Storage
  extracted_text_path       TEXT,                       -- OCR/text extraction artifact path
  generated_schema_path     TEXT,                       -- generated schema artifact path
  rendered_template_path    TEXT,                       -- generated/rendered template path
  checksum                  TEXT,                       -- de-dup + change detection
  file_type                 TEXT,
  language                  TEXT,
  version_label             TEXT,
...
-- ----------------------------------------------------------------------------
-- 3. form_templates
--    Canonical, versioned, fillable form schemas rendered as UI components.
--    schema_json describes sections + fields; field_mappings_json maps intake
--    answers into fields; validation_rules_json drives the validation engine.
-- ----------------------------------------------------------------------------
...
-- Only one active version per document at a time (versioning guardrail).
CREATE UNIQUE INDEX IF NOT EXISTS uq_formtpl_active_per_doc
  ON form_templates (requirement_document_id)
  WHERE status = 'active';

-- ----------------------------------------------------------------------------
-- 4. user_form_instances
--    A user's in-progress / completed fill of a form template for a target.
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS user_form_instances (
  id                     UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id              UUID NOT NULL,
  target_id              UUID NOT NULL,                -- e.g. business/submission id
...
-- One instance per (target, template) so save/resume is idempotent.
CREATE UNIQUE INDEX IF NOT EXISTS uq_userforms_target_template
  ON user_form_instances (target_id, form_template_id);

-- ----------------------------------------------------------------------------
-- 5. requirement_monitoring_runs
--    One row per execution of the monitoring agent against a source.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 6. requirement_change_events
--    Detected changes awaiting admin review. Feeds the admin review queue.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Admin review queue: rules/documents/templates needing attention plus open
-- change events, in one place for the admin UI (PR 10).
-- ----------------------------------------------------------------------------
...
DROP VIEW IF EXISTS v_admin_review_queue;
