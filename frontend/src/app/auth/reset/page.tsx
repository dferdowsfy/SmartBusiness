...
export default function ResetPasswordPage() {
  ...
  if (!ready) return <div className="min-h-screen bg-[#f4f1ea] p-10 text-center text-[#161616]/50">Loading…</div>;

  return (
    <div className="min-h-screen bg-[#f4f1ea]">
      <div className="max-w-md mx-auto mt-12 bg-white border border-slate-200 rounded-2xl p-7">
        <h1 className="text-2xl font-bold text-[#161616]">Set a new password</h1>
        ...
        {!hasSession ? (
          <>
            ...
            <button onClick={() => router.push("/auth/login?mode=forgot")}
              className="w-full bg-[#161616] text-white rounded-lg py-2.5 font-medium mt-5">
          ...
          <>
            <p className="text-sm text-[#161616]/60 mt-1 mb-5">Choose a new password for your account.</p>
            <form onSubmit={submit} className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-[#161616]/70 mb-1">New password</label>
                <input type="password" required minLength={6} autoComplete="new-password"
                  value={password} onChange={(e) => setPassword(e.target.value)}
                  placeholder="At least 6 characters"
                  className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm text-[#161616] placeholder:text-[#161616]/40 bg-white" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-[#161616]/70 mb-1">Confirm password</label>
                <input type="password" required minLength={6} autoComplete="new-password"
                  value={confirm} onChange={(e) => setConfirm(e.target.value)}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm text-[#161616] placeholder:text-[#161616]/40 bg-white" />
              </div>
              <button type="submit" disabled={busy || password.length < 6}
                className="w-full bg-[#161616] text-white rounded-lg py-2.5 font-medium disabled:opacity-50">
