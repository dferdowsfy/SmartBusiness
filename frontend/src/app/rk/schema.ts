// ============================================================================
// Regulatory Knowledge Graph — idempotent schema bootstrap (SERVER ONLY).
//
// Same pattern as requirements/schema.ts: the DDL below is applied lazily,
// once per process, and every statement is CREATE ... IF NOT EXISTS so it is
// safe to run repeatedly. Mirrored to data/rk_schema.sql for manual import.
// All tables are rk_-prefixed to avoid collisions with the live requirement_*
// tables and the aspirational design/db schema.
// ============================================================================

import { getPool } from "../graph/db";

export const RK_SCHEMA_SQL = `
-- ============================================================
...
async function applySchema(): Promise<void> {
  const pool = getPool();
  if (!pool) return;
  await pool.query(RK_SCHEMA_SQL);
}

/** Apply the rk schema once per process; retries on the next call if it failed. */
export function ensureRkSchema(): Promise<void> {
  const pool = getPool();
