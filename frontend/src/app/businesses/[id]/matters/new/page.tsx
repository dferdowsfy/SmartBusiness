  ...
  return (
    <div className="min-h-screen bg-[#f6f8fb]">
      ...
      <main className="mx-auto max-w-2xl px-5 py-8">
        <Link href={`/businesses/${id}`} className="text-sm font-semibold text-[#0D9488]">← Business profile</Link>
        <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#0D9488]">New matter</p>
          <h1 className="mt-1 text-2xl font-bold text-[#0A2540]">Start a filing or renewal</h1>
          ...
          <form onSubmit={submit} className="mt-6 space-y-4">
            <label className="block"><span className="mb-1 block text-xs font-bold text-slate-600">Matter type</span><select value={matterType} onChange={(event) => setMatterType(event.target.value as MatterType)} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm text-[#0A2540]">{MATTER_TYPES.filter((type) => type !== "NEW_BUSINESS_FORMATION").map((type) => <option key={type} value={type}>{LABELS[type]}</option>)}</select></label>
            <label className="block"><span className="mb-1 block text-xs font-bold text-slate-600">Title <span className="font-normal text-slate-400">(optional)</span></span><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder={LABELS[matterType]} className="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm text-[#0A2540]" /></label>
            ...
            <button disabled={busy} className="w-full rounded-xl bg-[#0A2540] py-3 text-sm font-semibold text-white disabled:opacity-50">{busy ? "Starting filing…" : "Continue to SmartPR intake →"}</button>
          </form>
        </div>
      </main>
    </div>
